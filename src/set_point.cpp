...
// double pre_req_raw[3][3] = {0.0,}; //x, y, z, vel_x, vel_y, vel_z, acc_x,
// acc_y, acc_z
...
{
  if (formation == "diamond" || formation == "Diamond" ||
      formation == "DIAMOND")
  {
                      ...
                      swarm_ctrl_pkg::srvMultiSetPosLocal::Response& res)
{
  ...
  if (req.pos_flag)
  {
    if (req.x != pre_req_pos[0] | req.y != pre_req_pos[1] |
        req.z != pre_req_pos[2] | offset != pre_offset)
    {
                      ...
                      swarm_ctrl_pkg::srvMultiSetVelLocal::Response& res)
{
  ...
  if (req.vel_flag)
  {
    if (req.vel_x != pre_req_vel[0] | req.vel_y != pre_req_vel[1] |
        req.vel_z != pre_req_vel[2])
    {
...
bool multiSetRawLocal(swarm_ctrl_pkg::srvMultiSetRawLocal::Request &req,
        ...
        if(req.pos_flag){
                if (req.x != pre_req_raw[0] | req.y != pre_req_raw[1] | req.z !=
pre_req_raw[2] | req.offset != pre_req_raw[9]){
                        ...
                        req.offset = (req.offset > 0) ? req.offset :
(-1)*req.offset;
                        ROS_INFO("move(%lf, %lf, %lf) offset : %lf", req.x,
req.y, req.z, req.offset);
                        res.success = true;
                        else{
                                res.success = false;
                        }
        ...
        if(req.vel_flag){
                if (req.vel_x != pre_req_raw[3] | req.vel_y != pre_req_raw[4] |
req.vel_z != pre_req_raw[5]){
                        for (int i = 0; i < NUM_DRONE; i++){
                                l_raw[i].velocity.x = req.vel_x;
                                l_raw[i].velocity.y = req.vel_y;
                                l_raw[i].velocity.z = req.vel_z;
                        }
                        ROS_INFO("vel(%lf, %lf, %lf)", req.vel_x, req.vel_y,
req.vel_z);
        ...
        if(req.acc_flag){
                if (req.acc_x != pre_req_raw[6] | req.acc_y != pre_req_raw[7] |
req.acc_z != pre_req_raw[8]){
                        for (int i = 1; i < NUM_DRONE; i++){
                        ...
                        ROS_INFO("acc(%lf, %lf, %lf)", req.acc_x, req.acc_y,
req.acc_z);
...
int main(int argc, char** argv)
{
  ...
  ros::ServiceServer multi_set_pos_local_server =
      nh.advertiseService("multi_set_pos_local", multiSetPosLocal);
  ros::ServiceServer multi_set_vel_local_server =
      nh.advertiseService("multi_set_vel_local", multiSetVelLocal);
  // ros::ServiceServer multi_set_raw_local_server =
  // nh.advertiseService("multi_set_raw_local", multiSetRawLocal);
