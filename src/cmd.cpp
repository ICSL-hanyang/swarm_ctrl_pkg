...
ros::ServiceClient multi_setpoint_local_client;
	...
	swarm_ctrl_pkg::srvMultiMode::Response& res)
{
	...
	if (set_mode_client[NUM_DRONE].call(set_mode) && set_mode.response.mode_sent)
	{
		res.success = true;
	}
	else
	...
	multi_setpoint_local_client.call(p_msg);
	return true;
}
